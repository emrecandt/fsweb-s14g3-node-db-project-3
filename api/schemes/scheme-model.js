...
async function  findById(scheme_id) {
  ...
  */
  ...
  const idTable = await db("schemes as sc")
    .leftJoin("steps as st", "sc.scheme_id", "st.scheme_id")
...
if (idTable.length === 0) {
  ...
  return {
    "scheme_id": Number(scheme_id),
    "scheme_name": idTable[0].scheme_name,
    "steps": idTable.filter(r => r.step_id !== null).map((step) => 
...
async function findSteps(scheme_id) {
  const stepTable = await db("steps as st")
